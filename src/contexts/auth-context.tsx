...
interface AuthContextType {
    ...
    loading: boolean;
    error: string | null;
    signUp: (email: string, password: string, displayName: string) => Promise<void>; // функція реєстрації
...
export const AuthProvider: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
    ...
    const signUp = async (email: string, password: string, displayName: string) => {
        setError(null);
        try {
            const { user } = await createUserWithEmailAndPassword(auth, email, password); // створення акаунту
